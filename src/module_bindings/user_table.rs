...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
    ...
    _table.add_unique_constraint::<i32>("friend_code", |row| &row.friend_code);
}
...
/// Access to the `friend_code` unique index on the table `user`,
/// which allows point queries on the field of the same name
/// via the [`UserFriendCodeUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.user().friend_code().find(...)`.
pub struct UserFriendCodeUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<User, i32>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> UserTableHandle<'ctx> {
    /// Get a handle on the `friend_code` unique index on the table `user`.
    pub fn friend_code(&self) -> UserFriendCodeUnique<'ctx> {
        UserFriendCodeUnique {
            imp: self.imp.get_unique_constraint::<i32>("friend_code"),
            phantom: std::marker::PhantomData,
        }
    }
}

impl<'ctx> UserFriendCodeUnique<'ctx> {
    /// Find the subscribed row whose `friend_code` column value is equal to `col_val`,
    /// if such a row is present in the client cache.
